...
function readVersion(): string {
  try {
    // eslint-disable-next-line no-restricted-syntax -- package.json, идентификаторов Директа здесь нет
...
function reg(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- схема инструмента известна только в рантайме; уйдёт с defineTool (docs/architecture.md)
  handler: (params: any) => Promise<string>
): void {
  server.registerTool(
    name,
    { title, description, inputSchema, annotations },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- то же: типы SDK не выводят params из inputSchema
    (async (params: any) => ({
      content: [{ type: "text" as const, text: await handler(params) }]
    })) as any // eslint-disable-line @typescript-eslint/no-explicit-any -- registerTool перегружен, вывод типа не сходится
  )
...
async function main() {
  ...
  const mode = process.env.YANDEX_DIRECT_SANDBOX === "1" || process.env.YANDEX_DIRECT_SANDBOX === "true"
    ? " [SANDBOX]" : ""
  console.error(`[yd-mcp] v${VERSION} запущен${mode}. ${registered.length} инструментов. Требуется YANDEX_DIRECT_TOKEN.`)
}
