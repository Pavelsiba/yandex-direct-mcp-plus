...
export const createTextAdSchema = z.object({
  ...
  title: z
    ...
    .max(AD_TEXT_LIMITS.title, { error: `Заголовок длиннее ${AD_TEXT_LIMITS.title} символов` })
    .meta({ description: `Заголовок объявления, до ${AD_TEXT_LIMITS.title} символов` }),
  title2: z
    .string()
    .max(AD_TEXT_LIMITS.title2, { error: `Второй заголовок длиннее ${AD_TEXT_LIMITS.title2} символов` })
    .optional()
    .meta({ description: `Второй заголовок, до ${AD_TEXT_LIMITS.title2} символов` }),
  text: z
    .string()
    .min(1, { error: "Текст не может быть пустым" })
    .max(AD_TEXT_LIMITS.text, { error: `Текст длиннее ${AD_TEXT_LIMITS.text} символов` })
    .meta({ description: `Текст объявления, до ${AD_TEXT_LIMITS.text} символов` }),
  href: z.string().min(1, { error: "Ссылка не может быть пустой" }).meta({ description: "Ссылка на сайт" })
})
...
export const moderateAdsSchema = z.object({
  ad_ids: z
    .array(idField("ID объявления"))
    .min(1, { error: "Список объявлений пуст" })
    .max(MAX_ADS_PER_MODERATION, { error: `За один вызов допустимо не больше ${MAX_ADS_PER_MODERATION} объявлений` })
