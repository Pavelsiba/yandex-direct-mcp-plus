...
export const getAccountBalanceSchema = z.object({
  logins: z
    .array(z.string())
    .optional()
    .describe("Логины аккаунтов (для агентств). По умолчанию — аккаунт токена (или YANDEX_DIRECT_LOGIN).")
})
...
export async function handleGetAccountBalance(params: z.infer<typeof getAccountBalanceSchema> = {}): Promise<string> {
  ...
  const logins = params.logins ?? (process.env.YANDEX_DIRECT_LOGIN ? [process.env.YANDEX_DIRECT_LOGIN] : undefined)
