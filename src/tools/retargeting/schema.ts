...
export const listRetargetingListsSchema = z.object({
  retargeting_list_ids: z
    ...
    .max(MAX_IDS_PER_CALL, { error: `За один вызов допустимо не больше ${MAX_IDS_PER_CALL} условий` })
    .optional()
...
const ruleArgument = z.object({
  ...
  membership_life_span: z
    .number()
    .int()
    .min(1, { error: "Срок не меньше одного дня" })
    .max(RETARGETING_LIMITS.membershipDays, { error: `Срок не больше ${RETARGETING_LIMITS.membershipDays} дней` })
    .optional()
    .meta({
...
const rule = z.object({
  ...
  arguments: z
    .array(ruleArgument)
    .min(1, { error: "Правило без условий" })
...
export const addRetargetingListSchema = z.object({
  name: z
    ...
    .max(RETARGETING_LIMITS.name, { error: `Название длиннее ${RETARGETING_LIMITS.name} символов` })
  ...
  description: z
    .string()
    .max(RETARGETING_LIMITS.description, { error: `Описание длиннее ${RETARGETING_LIMITS.description} символов` })
    .optional()
    .meta({ description: "Описание условия — видно только в интерфейсе, на показы не влияет" }),
  rules: z
    .array(rule)
    .min(1, { error: "Условие без правил" })
