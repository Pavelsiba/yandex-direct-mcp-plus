...
export const addAdExtensionsSchema = z.object({
  callouts: z.array(z.string().min(1).max(25)).min(1).max(1000).describe("Тексты уточнений, каждый до 25 символов")
})

export async function handleAddAdExtensions(params: z.infer<typeof addAdExtensionsSchema>): Promise<string> {
  return formatResult(
    await apiPost("adextensions", "add", {
...
export async function handleDeleteAdExtensions(params: z.infer<typeof deleteAdExtensionsSchema>): Promise<string> {
  return formatResult(
    await apiPost("adextensions", "delete", {
