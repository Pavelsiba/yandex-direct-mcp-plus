...
export const listAdExtensionsSchema = z.object({
  ad_extension_ids: z
    ...
    .check(z.maxLength(MAX_IDS_PER_CALL, { error: `За один вызов допустимо не больше ${MAX_IDS_PER_CALL} уточнений` }))
    .optional()
  ...
  callouts: z
    .array(
      z
        .string()
        .check(
          z.minLength(1, { error: "Текст уточнения не может быть пустым" }),
          z.maxLength(CALLOUT_TEXT_MAX, { error: `Текст уточнения длиннее ${CALLOUT_TEXT_MAX} символов` })
        )
    )
    .check(
      z.minLength(1, { error: "Список уточнений пуст" }),
      z.maxLength(MAX_CALLOUTS_PER_CALL, {
        error: `За один вызов допустимо не больше ${MAX_CALLOUTS_PER_CALL} уточнений`
      })
    )
