...
const adImageHash = z.string().min(1, { error: "Хеш изображения не может быть пустым" })

const adImage = z.object({
  image_data: z
    .string()
    .min(1, { error: "Данные изображения не могут быть пустыми" })
    .meta({ description: "Содержимое файла изображения в base64" }),
  name: z
    .string()
    .min(1, { error: "Название не может быть пустым" })
    .max(AD_IMAGE_NAME_MAX, { error: `Название длиннее ${AD_IMAGE_NAME_MAX} символов` })
...
export const manageAdImagesSchema = z.object({
  ...
  images: z
    .array(adImage)
    .min(1, { error: "Список изображений пуст" })
    .max(MAX_IMAGES_PER_CALL, { error: `За один вызов допустимо не больше ${MAX_IMAGES_PER_CALL} изображений` })
    .optional()
    .meta({ description: "Изображения для загрузки; обязателен при action=add" }),
  ad_image_hashes: z
    .array(adImageHash)
    .min(1, { error: "Список хешей пуст" })
    .max(MAX_IDS_PER_CALL, { error: `За один вызов допустимо не больше ${MAX_IDS_PER_CALL} хешей` })
    .optional()
